...
| 初始化 Monitor 看板 | `cd 监测者/monitor && npm install`，启动 `node src/server.js` | ✓（可选） |
...
# 检查 Monitor 仪表盘（可选）
curl http://localhost:3456/api/health  # 应返回 {"status":"ok"}
```
...
| `kf-model-router` | — | 省 | 独立 | 无（被 kf-model-router 按 adapter 引用） | kf-model-router | flash |
| `kf-model-router` | — | 省 | 独立 | 无（被 kf-model-router 按需引用安全模块） | kf-model-router | flash |
...
| ~~`asta-skill`~~ (已移除) | Agents365-ai | — | — | 目录为空，功能未就绪，已移除 |

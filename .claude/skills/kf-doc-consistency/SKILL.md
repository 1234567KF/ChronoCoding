...
description: |
  Checks global document consistency across the project after a new skill is installed, modified, or removed. Verifies every skill reference, trigger word, directory tree listing, and calling chain is consistent across CLAUDE.md, 安装配置文档相关/AICoding.md, INSTALL.md, and MANUAL.md.
...
Stage 2 ─── Check 安装配置文档相关/AICoding.md ─── 对比详细技能表 + 调用链 + FAQ
...
### 1f. 安装配置文档相关/README.md — 触发词表 + 目录树 + 第三方集成

**定位**：`安装配置文档相关/README.md` 中 `## 功能触发词` 和 `## 目录结构` 和 `## 第三方开源集成` 三节。
...
## Stage 2 — Check 安装配置文档相关/AICoding.md

读取 `安装配置文档相关/AICoding.md`，对以下各节执行一致性检查。
...
> 技能有增减，请手动重截海报：打开 `安装配置文档相关/assets/posters/宣传海报_浅色.html` → 截图 → 覆盖 `宣传海报_浅色.png`

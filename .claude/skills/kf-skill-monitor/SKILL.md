...
# kf-skill-monitor 技能调用链路追踪 + Token成本追踪

> 追踪 `.claude/skills/` 下**所有技能**的调用 — kf-定制 + 通用下载 + MCP工具
> **v3.0** 新增：Token成本追踪、缓存命中/未命中区分、输入/输出分离计算

## 触发词

`/skill-monitor`、`skill-monitor`、`技能监控`、`使用率`、`调用链路`、`技能清单`、`token成本`、`cost`
...
```bash
node .claude/helpers/skill-monitor.cjs inventory
```
...
```bash
node .claude/helpers/skill-monitor.cjs cost
```
...
```bash
node .claude/helpers/skill-monitor.cjs tree
```
...
```bash
node .claude/helpers/skill-monitor.cjs report
```

输出到 `token测评/skill-usage-report.md`，包含：
...
```json
{
  ...
  "hooks": [
    ...
    { "command": "...skill-monitor.cjs pre-tool" }
  ]
}
```
...
| 文件 | 位置 | 用途 |
|------|------|------|
| skill-monitor.cjs | .claude/helpers/ | 核心监控脚本 v3.0 |
| skill-traces.jsonl | .claude-flow/data/ | 实时调用日志（含token） |
| skill-inventory.json | .claude-flow/data/ | 技能清单（95个） |
| skill-usage-summary.json | .claude-flow/data/ | 汇总数据（含token统计） |
| skill-usage-report.md | token测评/ | 人可读报告 |
| SKILL.md | .claude/skills/kf-skill-monitor/ | 本文件 |
...
| cost-tracker.cjs | skill-traces 是 cost-tracker 的技能维度补充 |
| metrics-db.mjs | skill-traces 是 metrics 细分维度 |

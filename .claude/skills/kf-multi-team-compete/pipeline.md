...
夯 = 红蓝绿三队并发竞争。每队内部不是散兵游勇，而是 **7 阶段流水线**，阶段间有明确门控和产物交接。三队流水线并行执行，互不阻塞。
...
每个团队自动执行以下 7 阶段流水线。**上游阶段输出自动成为下游阶段输入。禁止跳过。**

```
Pre-Stage     Stage 0        Stage 0.5       Stage 1        Stage 2          Stage 3        Stage 4        Stage 5
 物料准备  →   需求对齐  →   测试设计先行 →   架构设计   →   TDD 微循环   →    集成测试   →    代码审查   →    方案汇总
 (协调者)      (全栈开发)     (QA 测试设计)    (全栈开发)     (全栈开发)        (集成测试)      (集成测试)     (前端设计师)
    │              │              │              │              │              │              │              │
    ▼              ▼              ▼              ▼              ▼              ▼              ▼              ▼
 PRD文档       对齐记录      测试文件        架构方案    代码+测试报告    覆盖率报告       审查报告       团队方案
```
...
| Stage 0.5 | QA 测试设计 | 对齐记录 + PRD/Spec | {team}-05-tests/ + {team}-05-red-report.md | 测试编译成功 + 全部 RED + 覆盖 3 维度 (与 Stage 1 并行) |
| Stage 1 | 全栈开发 | 对齐记录 | {team}-01-architecture.md | 架构方案无歧义 |
| Stage 2 | 全栈开发 | 架构方案 + Stage 0.5 测试文件 | 代码 + {team}-02-tdd-cycle-N.md + {team}-02-implementation.md | 全部测试 GREEN + 分支覆盖 ≥ 70% + checklist A-K 类通过 |
| Stage 3 | QA 专家 | 代码 + Stage 2 TDD 报告 + Stage 0.5 测试矩阵 | {team}-03-test-report.md（含覆盖率数据） | Happy Path 通过 + 分支覆盖 ≥ 70% + checklist A/B/D/F/G/J/K 类专项测试通过 |
| Stage 4 | Code Review 专家 | 代码 + 测试报告 + 覆盖率数据 | {team}-04-review-report.md（含 TDD 合规审计） | 无 error + TDD 合规通过 + checklist 审计通过（自检+测试+审查+TDD 四重确认） |
...
graph LR
    ...
    S0 --> IG{反转门控}
    IG -->|零问题/已回答| S05[Stage 0.5 测试设计]
    IG -->|有问题| Q[向用户提问]
    Q -->|用户回答| S05
    S05 --> S1[Stage 1 架构设计]
    S05 -.->|并行| S1
    S1 --> S2[Stage 2 TDD 微循环]
    S05 --> S2
    S2 --> S3[Stage 3 集成测试+覆盖率]
    S3 --> S4[Stage 4 代码审查+TDD审计]
    S4 --> S5[Stage 5 方案汇总]
```

Stage 0.5 和 Stage 1 可并行执行——测试设计关注"要建什么"，架构设计关注"怎么建"。
Stage 2 须等待 Stage 0.5 和 Stage 1 都完成后才开始（需要两份输入）。
...
| 特性 | 说明 |
|------|------|
| 并行度 | 3 队 × 7 阶段 = 最多 21 个 agent 同时运行（Stage 0.5 与 Stage 1 并行，可同队 2 agent 并发） |
...
| Agent | 流水线阶段 | 模型 |
|-------|-----------|------|
| 全栈开发 | Stage 0（对齐）+ Stage 1（架构）+ Stage 2 TDD 微循环（编码） | sonnet (flash) |
| QA 测试设计 | Stage 0.5（测试设计先行 — 与 Stage 1 并行） | sonnet (flash) |
| QA 专家 | Stage 3-4（测试+覆盖率门控→审查） | sonnet (flash) |
...
| 任务类型 | Agent 配置 | 流水线策略 |
|----------|-----------|-----------|
| **编码开发** | 3 agent/队 × 3 队 + 裁判 + 汇总 + 对抗者 = 12 | 完整 6 阶段 + 对抗质疑 |
...
|-----------|---------|-------------|------|
| **指定项目代码** | 用户提供本地路径，含 .claude/ skills/ rules/ 等 | `kf-code-review-graph` 提取依赖图谱 + `kf-kb-envoy` 摄入知识库 | `{team}-00-research.md` — 代码结构、技能清单、配置规则摘要 |
...
| Phase 2续 | Stage 0.5（与 Stage 1 并行）→ Stage 1-5 继续执行 | 全部完成 → 进入 Phase 3 |
...
# TDD 专用 gate check
node .claude/helpers/tdd-gate-check.cjs --stage 0.5 --team <队名> --check-red    # Stage 0.5 RED 验证
node .claude/helpers/tdd-gate-check.cjs --stage 2 --team <队名> --check-green    # Stage 2 GREEN 验证
node .claude/helpers/tdd-gate-check.cjs --stage 4 --team <队名> --audit-tdd      # Stage 4 TDD 合规审计
...
Phase 2续 并发执行 (Stage 0.5 + Stage 1 并行 → Stage 2-5)
  ├── [并行] 红队 Pipeline: Stage0.5(测试设计) ∥ Stage1(架构) → Stage2(TDD)→...→Stage5
  ├── [并行] 蓝队 Pipeline: Stage0.5(测试设计) ∥ Stage1(架构) → Stage2(TDD)→...→Stage5
  └── [并行] 绿队 Pipeline: Stage0.5(测试设计) ∥ Stage1(架构) → Stage2(TDD)→...→Stage5
       │
       └── 每队门控: 阶段产物必须完整 + TDD 门控通过，否则阻塞该队

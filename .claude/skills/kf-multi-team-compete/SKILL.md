...
**正确的链路**：`SDD Excel → kf-prd-generator → PRD.md → kf-spec → Spec → spec-reviewer 审查 → 夯 并发执行`

**注意**：即使无 SDD Excel，只要任务描述模糊（如一句话需求），协调者也应主动建议用户先走 kf-prd-generator 做需求结构化，或走 kf-spec 做 Spec 驱动开发，再进入 `/夯` 并发竞争。

**Spec 质量门控**：Spec 生成后使用 `node .claude/helpers/spec-reviewer.cjs review <spec-path>` 自动审查，5 维评分（AC 可测性/边界覆盖/依赖完整/结构清晰/功能完整），≥70 分且无 P0 缺陷为通过。上限 3 轮修复循环。
...
| Round 3 仍有 P0 | ⚠️ 标记 UNRESOLVED，写 `.claude/logs/escalation.jsonl`，携带警告进入 Stage 3.5 |
| Round 3 无 P0 | ✅ 通过 |
...
- **动作**：
  ...
  5. **Fix Protocol 记录** — 若发现运行时错误，按 fix-protocol 格式记录 bug 类型 + 修复方案（见 .claude/fix-protocol/）
...
### 5. Fix Protocol 记录
{若无运行时错误：无记录}
{若有：已写入 .claude/fix-protocol/ 的记录条目}
```
...
处理规则：
- **小范围变更**（1-2 个文件修改）：直接通知对应专家，不重新 spawn
...
每队 3-5 个 agent 分工，按需 spawn，外加独立角色：
...
```
node .claude/helpers/hang-state-manager.cjs --init "{任务名}" --depth {A|B|C}
```
...
```
node .claude/helpers/hang-state-manager.cjs --recovery
```
若返回 `"needed": true`，展示恢复看板让用户选择。
若返回 `"needed": false`，直接进入正常流程。

- 选 A → 不调用技能，在当前对话中继续
- 选 B → `node .claude/helpers/hang-state-manager.cjs --handoff`，提示用户执行 /gspowers
- 选 C → 从当前阶段恢复 Pipeline，执行完整编码流程

### Phase 1.4 — Team Lead 出计划 + 用户审核

> **核心理念**：Team Lead 先输出执行计划，用户审核批准后才 spawn 三队并发执行。
...
3. 并行 spawn 三队的 Stage 0 agent（run_in_background: true, model: "sonnet"）
   每个 agent spawn 后 MUST 调用:
     node .claude/helpers/hammer-bridge.cjs agent-spawn --team <红/蓝/绿> --agent <agent名> --task-id <阶段> [--session-id <ID>]
   每个 agent 的 prompt 中 MUST 包含：
     ...
     - **重试与容错指令**：失败时 MUST 记录具体错误原因；失活 5min 会被自动终止重试
	   ...
	   - **浅层 Plan 预览**：共享前缀之后、角色定义之前 MUST 注入 10-15 行任务拆解预览。使用以下命令生成并注入：
	     ...
	     | # | 模块 | 动作 | 预估文件 | 风险 |
	     | 1 | auth | 新增JWT验证 | +3 | 低 |
	     预估总影响: 8文件 ~200行 | 执行: ~3min | 模式: balanced
	     回复"改"可暂停修订，30s后自动继续...
	     ```
	     人类打断窗口: 30s超时自动继续。浅层 Plan 内容在 `### SHARED PREFIX END` 之后、角色定义之前注入
   ⚠️ 缓存优化：参考 `.claude/rules/cache-optimization.md` 全局规则。
     共享前缀 MUST 逐字相同（包括空格和换行），严格控制在 300-500 token 内。
     价格杠杆：Pro ¥3/M(未命中) vs ¥0.025/M(命中) — 120x 差价；Flash ¥1/M vs ¥0.02/M — 50x 差价。
     三种落盘时机：请求结束落盘 / 固定 token 间隔 checkpoint / 前缀检测恢复。
     预热策略：spawn 第一个 agent 前先发一次带最长文档的请求，让共享前缀落盘。
     监控：从 API 响应读 usage.prompt_cache_hit_tokens / prompt_cache_miss_tokens，< 30% 告警。
     收益：18 agent 并发场景成本可降 ~83%（详见 cache-optimization.md 收益估算）。
   模型路由：
     - 前端专家 agent → model: "sonnet"（flash, 执行层面）
     - 后端专家 agent → model: "sonnet"（flash, 执行层面）
...
3.5. 【硬 Gate 2.0 — 反转门控】三队 Stage 0 全部完成后 MUST 执行硬性阻断检查。Gate 状态机: IDLE→SCANNING→[有CRITICAL?→WAITING_ANSWER→BROADCAST]→PASSED。零问题零延迟自动通过。
	     ...
	     **执行**: `node .claude/helpers/gate-executor.cjs --scan {红队文件} {蓝队文件} {绿队文件} --task "{任务名}"`
	     退出码: 0=通过 | 2=阻断。阻断时读取 stdout 的问卷 JSON，用 AskUserQuestion 展示。
	     **回答**: `echo '{"Q-id":"A"}' > .claude-flow/gate-answers.json && node .claude/helpers/gate-executor.cjs --answer .claude-flow/gate-answers.json`
	     **spawn检查**: `node .claude/helpers/gate-executor.cjs --check-spawn` (0=放行 1=阻断)
	     **约束注入**: `node .claude/helpers/gate-executor.cjs --constraint-prompt`
	     详见下方 Gate 规格。
     ...
     3. 阻断 Pipeline — spawnStage1() MUST 检查 gate 状态，非 PASSED 则 throw Error
     ...
     - 写日志: .claude/logs/inversion-gate.jsonl
     ...
     // 读取 broadcast 答案注入 prompt → 正常 spawn

     失活保护:
     - Gate 状态持久化 .claude-flow/gate-state/ + .claude-flow/gate-broadcast/
     - Team Lead 会话重置后 Gate 状态不丢失
     - 新 spawn agent 从 broadcast 文件读取答案

     门控规则: spawnStage1 前置检查 | 扫描 10s 内完成 | 用户 5min 超时 | 零问题零延迟 | 每次写 inversion-gate.jsonl
...
5. Pipeline 继续推进：Stage 3→Stage 3.5→...→Stage 5（TDD 全 GREEN + 覆盖率达标后全自动）
   ...
   同时每阶段 agent MUST 发送 A2A 通知:
     node .claude/helpers/hammer-bridge.cjs a2a-notify --type status --team <队> --agent <名> --status <done|blocked|failed> --message "<摘要>" [--artifact <产物>]
...
for team in red blue green; do
  node .claude/helpers/quality-signals.cjs --list | grep "kf-code-review-graph" | grep "$team"
done
```
...
```
红队 review: .claude/logs/review-{ts}-{exec_id}.json → severity_distribution
蓝队 review: .claude/logs/review-{ts}-{exec_id}.json → severity_distribution
绿队 review: .claude/logs/review-{ts}-{exec_id}.json → severity_distribution
```
...
// 写入 .claude-flow/hang-state.json
{
  ...
  "artifacts": {
    "red_review": ".claude/logs/review-{ts}-{id}.json",
    "blue_review": ".claude/logs/review-{ts}-{id}.json",
    "green_review": ".claude/logs/review-{ts}-{id}.json"
  }
}
```
...
裁判评分 → 计算置信度
  ...
  │
  ├── 0.70-0.84 → ⚠️ pass@2
  │   ├── spawn 2 个独立编码 agent（相同架构方案，不同随机 seed）
  │   ├── 两路并行编码 → 产出 2 份独立代码
  │   ├── 裁判对 2 份新代码分别评分（仅 Stage 2/3 相关维度）
  │   └── 取置信度最高版本 ← 进入 Phase 4
  │
  ├── 0.50-0.69 → 🔴 pass@3
  │   ├── spawn 3 个独立编码 agent（相同架构方案，不同随机 seed）
  │   ├── 三路并行编码 → 产出 3 份独立代码
  │   ├── 裁判对 3 份新代码分别评分
  │   └── 取置信度最高版本 ← 进入 Phase 4
  │
...
- 多个团队同时触发 pass@k 时，协调者应并行 spawn 所有重试 agent，不串行等待
- 置信度评分由裁判在评分同时完成，不额外 spawn agent
...
协调者读取终版方案 + 代码产物
  ...
  │    抽取：目录树骨架 + 文件命名规范
  │    写入：.claude/templates/{框架}-{类型}-structure.md
  │
  ├── 2. 组件/模块模板
  │    检测：是否有可复用的组件或模块模式（表单、列表、弹窗、API 调用等）
  │    抽取：组件接口签名 + 核心逻辑 + 状态管理 + 错误处理
  │    写入：.claude/templates/{框架}-{组件名}.md
  │
  ├── 3. API 调用模式
  │    检测：是否有标准的 API 调用模式（请求/响应/错误处理）
  │    抽取：API client 配置 + 请求封装 + 错误处理 + 类型定义
  │    写入：.claude/templates/{语言}-api-pattern.md
  │
  ├── 4. 配置模板
  │    检测：是否有工具/框架配置文件（tsconfig、vite.config、docker-compose 等）
  │    抽取：关键配置项 + 最佳实践设置
  │    写入：.claude/templates/{工具}-config.md
  │
  └── 5. 模板索引更新
      追加到 .claude/templates/INDEX.md（模板名称 + 说明 + 适用场景 + 来源任务）
```
...
```
MUST 检查 .claude/templates/INDEX.md，若发现与本任务匹配的模板：
1. 用 ctx_read 读取匹配模板
2. 在编码时遵循模板中的架构和模式
...
#### Gotchas
- Phase 6.5 是可选增强阶段，协调者执行时不 spawn 额外 agent，直接在本会话中完成
...
agent:
  max_concurrent_agents: 12        # 全局最大并发数
...
hammer-watch 后台守护（`hammer-watch.cjs`）只做状态管理（扫描队列、失活检测、写就绪文件），**不 spawn agent**。实际的任务处理、Agent spawn、流水线编排由技能内的 Watch 循环负责。
...
SKILL.md Watch 循环 (对话内, /夯 --watch 触发)
  ...
  ├── Phase 1-6 完整流水线执行
  ├── spawn Agent (Agent tool, run_in_background)
...
φ 循环入口
  ...
  │    └── Phase 2-6 并发执行 → 归档到 hammer-artifacts/
  │
  ├─ 4. 若有到期重试 → spawn 重试 agent
  │    node .claude/helpers/hammer-bridge.cjs retry
  │
...
async function watchLoop() {
  ...
  // 2. 标记为 watch 模式
  exec(`node .claude/helpers/hammer-bridge.cjs watch --interval ${interval}`);
  ...
  while (true) {
    ...
    // 6. 处理到期重试
    exec('node .claude/helpers/hammer-bridge.cjs retry');
...
node .claude/helpers/hammer-bridge.cjs a2a-task --description "任务描述" --from-agent "red/fullstack" --priority 1

# 或直接写 JSON 到队列目录
echo '{"task":"描述","from":"red/fullstack","priority":1}' > .claude-flow/hammer-queue/task-$(date +%s).json
```

这样「有人指挥、有人干活」的 A2A 协作就闭环了——一个 Agent 发任务到队列，Watch 循环自动发现并 spawn 三队执行。
...
```bash
# Agent 失败后自动入队
node .claude/helpers/hammer-bridge.cjs agent-fail --team red --agent fullstack --error "turn timeout"

# 查看所有待重试任务
node .claude/helpers/hammer-bridge.cjs retry

# 手动触发特定 agent 立即重试
node .claude/helpers/hammer-bridge.cjs retry --team red --agent fullstack
...
- Stage 2 编码阶段前端设计师可与前端专家、后端专家并行（UI 原型 + 前端实现 + 后端实现独立推进）
...
- **pass@k 重试 agent 的 seed 必须不同**——相同 seed 会导致 LLM 输出高度趋同，失去多路独立的意义。协调者在 spawn 重试 agent 时追加随机 seed 参数。
...
- **Fix Protocol 每次 Stage 3/3.5 发现 P0 错误时自动记录**——用 `node .claude/helpers/hammer-bridge.cjs fix-record` 记录，后续 Stage 2 编码时自动检索匹配项并注入 prompt。
...
| Phase 6.5（条件触发） | 检查 .claude/templates/ 目录是否存在/更新；失败不阻断主流程 | 记录错误到日志，不阻塞输出摘要 |

验证原则：**Plan → Build → Verify → Fix** 强制循环，不接受主观"我觉得好了"。

## Harness 记忆持久化（铁律 4）

Phase 6 汇总者回应与执行完成后 MUST 将最终评分卡、对抗报告摘要和终版方案写入 `memory/hammer-results.md`，包含对抗者质疑记录和汇总者回应决策。Phase 6.5 模板抽取后 MUST 将 `.claude/templates/INDEX.md` 更新并记录新增模板摘要。
...
| `.claude/templates/` | Phase 6.5 | 模板自动抽取库，对标 OpenGame Template Skill |
| `.claude/fix-protocol/` | Stage 3.5 + Stage 3 | 活修复协议库，对标 OpenGame Debug Skill。`fix-record` 记录，`fix-search` 检索 |

...
>
> 详细原理和策略见：`.claude/rules/cache-optimization.md`
...
配置见 CLAUDE.md，技能见 .claude/skills/。
...
TDD 默认工作流：测试先行 → RED→GREEN→REFACTOR 微循环 → 覆盖率门控（≥70% 分支覆盖）。
...
## TDD 约束（编码阶段 agent 必须遵守）

TDD 是默认工作流。编码 agent MUST：
- Stage 0.5 生成的可执行测试文件是 Stage 2 编码的唯一验收标准
- 先读测试再写代码（测试先行原则）
- RED→GREEN→REFACTOR 微循环驱动开发
- 分支覆盖率目标 ≥ 70%（Stage 3 门控）
- 禁止 it.todo / 空断言 — 测试必须完整可执行
- 编码 checklist 新增 K 类（TDD 合规）：K1-K7
- 可用工具：Vitest --coverage | tdd-gate-check.cjs | coverage-reporter.cjs

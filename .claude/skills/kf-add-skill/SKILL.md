...
| Source | Command |
|--------|---------|
| GitHub repo | `git clone <url> .claude/skills/<skill-name>/` |
| npm package | `npm install <package> --prefix .claude/skills/<skill-name>/` |
| Single file | Download with curl to `.claude/skills/<skill-name>/` |
...
This project uses BOTH `.claude/` and `.trae/` for Claude Code and Trae Builder respectively.

### 3a. Register in .claude/

Create (or verify) the skill directory at `.claude/skills/<skill-name>/`. This is already done if you cloned there in Step 2.

If the skill source is elsewhere (e.g. global `~/.claude/skills/`), copy it:
```bash
...
1. Read `.trae/rules.md` to understand current layout
2. If the skill is a standalone tool (not a meta/process skill), add a reference entry under a new `## 可用技能速查` section or append to an existing skill list:
...
3. For skills that need more Trae integration, create `.trae/skills/<skill-name>/` with a condensed rules file if the skill has special constraints Trae should know about.

Gate: Do NOT proceed until both directories are updated.
...
Read `.claude/CLAUDE.md`. Add entry to the kf- series table (or create a new row for non-kf skills):
...
Scan ALL `SKILL.md` files in `.claude/skills/` to find:
...
- The `.trae/` directory currently contains only `rules.md`. Do NOT create `.trae/skills/` directory structure unless the skill has Trae-specific constraints beyond what `.trae/rules.md` already captures.
- When updating CLAUDE.md, match the EXACT table column count and format. A misaligned pipe character breaks the markdown table.
...
- Some skills are stored only globally (`~/.claude/skills/`) and symlinked. Check for symlinks before copying.
- kf-model-router is a meta-skill (auto-triggered by Hook, not by user). Never add it to user-facing trigger tables.

...
async function runTests() {
  ...
  console.log('\n【Test 1】ModelRegistry — 模型注册与查询');
  {
    ...
    // 1.1 注册了 5 个默认模型
    assertEqual(registry.list().length, 5, '注册了 5 个默认模型');
    ...
    const openaiModels = registry.list({ provider: 'openai' });
    assertEqual(openaiModels.length, 2, 'OpenAI 有 2 个模型');
  ...
  console.log('\n【Test 3】RoutingEngine — 路由决策');
  {
    ...
    assertInArray(simpleDecision.model.id,
      ['deepseek-v4-flash', 'openai-4o-mini'],
      '简单任务推荐轻量模型');
    ...
    assertInArray(qaDecision.model.id,
      ['openai-4o-mini', 'deepseek-v4-flash'],
      '简单问答推荐轻量模型');
  ...
  console.log('\n【Test 4】Dispatcher — 并发调度');
  {
    ...
    // 4.3 4o-mini → haiku 映射
    const miniResult = dispatcher.resolveAgentModel('openai-4o-mini');
    assertEqual(miniResult.model, 'haiku', '4o-mini 模型映射到 haiku');
  ...
  console.log('\n【Test 5】HealthChecker — 健康探测与断路器');
  {
    ...
    assert(summary.length === 5, '5 个模型的健康摘要');
  ...
  console.log('\n【Test 7】End-to-End — 完整流程');
  {
    ...
    assert(stats.models.length === 5, '初始化后 5 个模型');

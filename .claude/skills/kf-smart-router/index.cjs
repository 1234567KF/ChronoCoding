...
async function init(options = {}) {
  ...
  console.error(`[kf-smart-router] 初始化完成: ${modelCount} 个模型, 路由引擎就绪`);
}
...
async function route(opts = {}) {
  ...
  console.error(`[kf-smart-router] 任务分类: type=${taskProfile.type}, complexity=${taskProfile.complexity}, confidence=${taskProfile.confidence}`);

  // Step 2: 路由决策
  const decision = await engine.decide(taskProfile, strategy);
  console.error(`[kf-smart-router] 路由决策: 推荐=${decision.model.id}, 置信度=${decision.confidence}`);
  ...
  (async () => {
    try {
      switch (cmd) {
        ...
        case 'init':
          await init();
          console.error('[kf-smart-router] 初始化完成');
          break;
        default:
      ...
      console.error(`[kf-smart-router] 错误: ${err.message}`);
      process.exit(1);
    }
  })();
}

...
| `D:\AICoding\.claude\skills\kf-model-router\providers\*.cjs` | 红队 adapter 文件保持独立，通过 registry 引用 |
...
用户触发技能
  ...
  │   │    ├─→ 可选: kf-model-router/providers/*.cjs (adapter 加载)
  │   │    └─→ CJK-aware 复杂度估算
  │   └─→ 输出路由指令
```
...
### kf-model-router 独立性
红队的 `kf-model-router` 技能完全独立运行，不受本融合影响。蓝队通过 registry 的 `adapter` 字段选择性引用其 adapter 文件（路径 `skills/kf-model-router/providers/`），引用失败时静默降级，无硬依赖。

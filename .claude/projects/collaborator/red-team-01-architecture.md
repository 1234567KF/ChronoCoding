# kf-model-router — 全自动多模型智能调度系统架构
...
// 映射表 kf-model-router model id → Agent model string
modelMap: {
...
- **kf-model-router** 作为增强层，优先级高于 kf-model-router
- 当 kf-model-router 可用时，自动覆盖 kf-model-router 的路由决策
- 当 kf-model-router 不可用时，回退到 kf-model-router

### 8.2 与 /夯 (kf-multi-team-compete) 的集成

```
当前:   协调者指定 model: "sonnet" → deepseek-v4-flash
增强后: 协调者调用 kf-model-router → 动态决定 model 参数
...
在 settings.json 的 PreToolUse Skill matcher 中追加 kf-model-router hook：
...
```json
{
  ...
  "hooks": [
    ...
    {
      "type": "command",
      "command": "node .claude/skills/kf-model-router/smart-router-hook.cjs",
      "timeout": 5000
    }
  ]
}

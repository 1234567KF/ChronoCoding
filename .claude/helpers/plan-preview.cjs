 ...
 *
 * 用法:
 *   node .claude/helpers/plan-preview.cjs                     # 扫描全部信号生成 Plan
 *   node .claude/helpers/plan-preview.cjs --team red          # 按队伍过滤
 *   node .claude/helpers/plan-preview.cjs --skill kf-spec     # 按技能过滤
 *   node .claude/helpers/plan-preview.cjs --json              # JSON 输出
 *   node .claude/helpers/plan-preview.cjs --inject            # 输出可直接注入 prompt 的 markdown
 *
 * API:

 ...
 *
 * 用法:
 *   node .claude/helpers/spec-reviewer.cjs review <spec-path> [--round N] [--output <path>]
 *   node .claude/helpers/spec-reviewer.cjs check <spec-path>      # Quick check: 70% pass?
 *   node .claude/helpers/spec-reviewer.cjs score <spec-path>      # Score only
 *   node .claude/helpers/spec-reviewer.cjs diff <v1-path> <v2-path>  # Spec diff
 *
 * 评分维度:
 *   - AC 可测性 (25%): 验收条件是否可验证
 *   - 边界覆盖 (20%): 边界情况和异常路径

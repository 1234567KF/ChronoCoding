#!/usr/bin/env node
/**
 * kf-skill-monitor v3.0 - 技能调用链路追踪 + Token成本追踪
 * 
 * 追踪 .claude/skills/ 下所有技能（kf-定制 + 通用下载 + MCP工具）
 * 追踪输入/输出 token，区分缓存命中/未命中
 * 
 * Usage:
 *   node skill-monitor.cjs log       --agent <a> --skill <s> [options]
 *   node skill-monitor.cjs pre-tool  (hook: auto-capture any tool call via stdin)
 *   node skill-monitor.cjs status     (real-time status)
 *   node skill-monitor.cjs tree       (call chain tree)
 *   node skill-monitor.cjs report     (generate markdown report)
 *   node skill-monitor.cjs inventory  (scan all skills in .claude/skills/)
 *   node skill-monitor.cjs cost       (token cost summary)
 *   node skill-monitor.cjs reset      (clear logs)
 */
...
const REPORT_PATH = path.join(PROJECT_ROOT, "token测评", "skill-usage-report.md");
const SUMMARY_PATH = path.join(DATA_DIR, "skill-usage-summary.json");
const INVENTORY_PATH = path.join(DATA_DIR, "skill-inventory.json");

if (!fs.existsSync(DATA_DIR)) fs.mkdirSync(DATA_DIR, { recursive: true });

// ============================================================
// Skill Registry - scan all skills from .claude/skills/
// ============================================================
...
// ============================================================
function showStatus() {
  if (!fs.existsSync(LOG_PATH)) { console.log("[skill-monitor] no log"); return; }
  const entries = readLog();
  if (entries.length === 0) { console.log("[skill-monitor] empty log"); return; }
...
function showTree() {
  if (!fs.existsSync(LOG_PATH)) { console.log("[skill-monitor] no log"); return; }
  const entries = readLog();
  if (entries.length === 0) { console.log("[skill-monitor] empty"); return; }
...
function generateReport() {
  ...
  let md = "# kf-skill-monitor Report\n\n> Generated: " + now + " | Task: " + traceId + "\n\n";
  ...
  console.log("[skill-monitor] Report: " + REPORT_PATH);
}
...
function resetLog() {
  ...
  console.log("[skill-monitor] log reset");
}
...
if (cmd === "pre-tool") {
  ...
  switch (cmd) {
    ...
    default: console.log("Usage: node skill-monitor.cjs <log|pre-tool|status|tree|report|inventory|cost|reset>"); break;
  }
}

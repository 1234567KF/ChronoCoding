...
| `kf-kb-envoy`               | —                 | 准     | 独立                                                                                                                                               | Knowledge Base Envoy：知识库/raw/ → 知识库/wiki/ → CLAUDE.md 知识库全生命周期管理                                            |
...
│   ├── skill-validator.cjs    # SKILL.md 行为级验证框架（13 项测试）
│   ├── review-rerun-check.cjs # 条件重审触发判断（P0/P1 密度检测）
...
│   ├── key-isolator.cjs       # 多供应商密钥隔离 + Axios 客户端工厂
│   ├── rate-limiter.cjs       # 令牌桶限流（每供应商独立桶）
...
| ---------------------------------------------------------------------- | ------------------------- | ------ | ----------------------------------------------------------------- |
| `/go` / `/导航` / `/开始`                                        | kf-go                     | 快     | —                                                                |
| `/夯 [任务]`                                                         | kf-multi-team-compete     | 夯     | 自动调用 12+ 个子技能                                              |
...
| `摄入文件` / `ingest` / `lint` / `检查知识库` / `更新知识库` | kf-kb-envoy               | 准     | Knowledge Base Envoy：知识库/raw/ → 知识库/wiki/ → CLAUDE.md 知识库管理       |
...
| `进化` / `自我优化` / `evolve`                                   | kf-evolution              | 稳     | AI 自我优化与迭代改进：从执行历史学习，自动优化策略                  |
| `监测者` / `仪表盘` / `dashboard` / `token监测`                   | kf-monitor                | 测的准  | 监测者仪表盘：Token 追踪 + 重审触发检测 + 成本分析，独立于 Agent 自报 |
| `节省追踪` / `saver` / `成本节省`                                  | kf-saver                  | 省     | 会话成本节省追踪：自动记录每次 API 调用的 token 节省数据              |
...
判断是否涉及知识库领域
  ↓
是 → 优先查 知识库/wiki/（先读 concepts/index.md → 再读相关页面）
否 → 正常回答
  ↓
需要更多细节 → 再读 知识库/raw/ 源文件
```

## 知识库结构

```
知识库/raw/      ← 不可变的原始文件（人类放入，LLM 只读）
知识库/wiki/     ← LLM 生成和维护的结构化知识库
```
...
| 你说                           | 我会做                                                   |
| ------------------------------ | -------------------------------------------------------- |
| 问问题（涉及知识库领域）       | 自动查 知识库/wiki/ 再回答                                      |
| "摄入这个文件" / "ingest"      | 读 知识库/raw/ 新文件 → 更新 wiki 页 → 更新 index → 追加 log |
| "做 lint 检查" / "检查知识库"  | 扫描 知识库/wiki/ 找矛盾页/孤立页/过期断言/死链                 |
| "做分析报告" / "关于 X 的报告" | 查 知识库/wiki/ + 知识库/raw/ → 生成结构化分析报告                    |
| "更新知识库"                   | 扫描 知识库/raw/ 新文件做 ingest → 跑 lint → 报告状态         |

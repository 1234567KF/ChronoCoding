...
### 步骤 7.5：初始化 Token 追踪监控看板

Monitor 是 Token 用量追踪的 Web 看板（端口 3456），为 kf-token-tracker 提供可视化仪表盘。
...
# 3. 启动监控看板（后台运行）
# Windows:
Start-Process -WindowStyle Hidden -FilePath "node" -ArgumentList "src/server.js"
# Linux/macOS:
# nohup node src/server.js > /dev/null 2>&1 &

Write-Host "Monitor 仪表盘: http://localhost:3456"
```

**工作原理**：
- Claude Code hooks（`monitor-session.cjs`、`monitor-hooks.cjs`、`token-accum.cjs`）自动推送会话和 Token 数据到 monitor
- 即使 monitor 未运行，数据会暂存到 `.claude-flow/data/pending-sessions/`，monitor 启动后自动同步
- 查看地址：`http://localhost:3456`

**注意**：Monitor 是可选组件。如果不需要实时 Token 追踪看板，可以跳过此步，kf-token-tracker 仍可通过 CLI 查询。
...
# 复制 Pipeline 扩展
	$patchDir = "D:\path\to\AICoding\.claude\skills\kf-gspowers-pipeline-patch"
...
# 模板文件: 安装或更新/templates/settings.json.template
...
# [步骤 5] 创建全局配置
# 模板文件: 安装或更新/templates/config.yaml.template
...
# 模板文件: 安装或更新/templates/tdd-config.yaml.template
...
# 7.4 创建全局 Git hooks
# 模板文件: 安装或更新/templates/pre-commit.template
...
>
> **补丁位置**：`.claude/skills/kf-gspowers-pipeline-patch/`
...
```
.claude/skills/kf-gspowers-pipeline-patch/
├── pipeline.md           # 新增文件内容
...
```powershell
# 运行安装脚本
./.claude/skills/kf-gspowers-pipeline-patch/install-pipeline.ps1
...
```
请根据 .claude/skills/kf-gspowers-pipeline-patch/execute-patch.md 的说明

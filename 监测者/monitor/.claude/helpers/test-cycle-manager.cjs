 ...
 *
 * 用法:
 *   node .claude/helpers/test-cycle-manager.cjs matrix <team> [--output <path>]
 *   node .claude/helpers/test-cycle-manager.cjs round <team> <round-N> [--output <path>]
 *   node .claude/helpers/test-cycle-manager.cjs check <team> [--round N]
 *   node .claude/helpers/test-cycle-manager.cjs issues <team> [--json]
 *   node .claude/helpers/test-cycle-manager.cjs fix <team> <round-N> --issues "<file>"
 *   node .claude/helpers/test-cycle-manager.cjs status <team>
 *
 * 测试矩阵维度:
 *   - 角色: 管理员、普通用户、游客
 *   - 权限: 全部权限、部分权限、无权限

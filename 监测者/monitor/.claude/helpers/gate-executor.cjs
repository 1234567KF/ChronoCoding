#!/usr/bin/env node
/**
 * gate-executor.cjs — 反转门控硬性阻断自动化执行器
 *
 ...
 *
 * 用法:
 *   node .claude/helpers/gate-executor.cjs --scan <red-alignment.md> <blue-alignment.md> <green-alignment.md>
 *   node .claude/helpers/gate-executor.cjs --status              # 查看当前 Gate 状态
 *   node .claude/helpers/gate-executor.cjs --answer <json-file>  # 用户回答问卷
 *   node .claude/helpers/gate-executor.cjs --check-spawn          # spawnStage1 前置检查
 *   node .claude/helpers/gate-executor.cjs --reset               # 重置 Gate 状态
 *
 * API:

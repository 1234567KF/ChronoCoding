...
```javascript
// Output JSON to .claude/logs/review-{timestamp}-{execution_id}.json
const reviewJSON = {
  review_report: {
    timestamp: "ISO8601",
    branch: "{branch}",
...
// Write to .claude/logs/review-{timestamp}-{execution_id}.json
// MUST use mkdir -p .claude/logs/ first
```

**JSON 输出后 MUST 调用 quality_signals 注入**：

```bash
node .claude/helpers/quality-signals.cjs --from-review <review-json-path> \
...
```bash
node .claude/helpers/review-rerun-check.cjs <review-json-path> --round <N>
```
...
当第 3 轮重审后仍满足触发条件时：
1. 标记为 UNRESOLVED，写入 `.claude/logs/escalation.jsonl`
...
每次重审判断结果写入 `.claude/logs/review-rerun.jsonl`，用于：
- 追踪触发频率和误触发率
- 分析代码质量趋势
- 人工抽查决策正确性（目标：正确率 > 95%）
...
7. **审查完成后 MUST 输出 JSON 到 `.claude/logs/review-{timestamp}-{execution_id}.json`**

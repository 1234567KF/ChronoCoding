...
选择完成后 MUST 写入 .claude-flow/hang-state.json：
...
Team Lead 启动时 MUST 检测 .claude-flow/hang-state.json：
若存在且 depth !== "C" 或 current_phase !== "done"：
...
3. 并行 spawn 三队的 Stage 0 agent（run_in_background: true, model: "sonnet"）
   ...
   每个 agent 的 prompt 中 MUST 包含：
     ...
     - **重试与容错指令**：失败时 MUST 记录具体错误原因；失活 5min 会被自动终止重试
	   - **浅层 Plan 预览**：共享前缀之后、角色定义之前 MUST 注入 10-15 行任务拆解预览（P0.6实现后替换为 plan_preview() 调用）。格式：
	     ```
	     ## 任务拆解预览
	     | # | 模块 | 动作 | 预估文件 | 风险 |
	     | 1 | auth | 新增JWT验证 | +3 | 低 |
...
3.5. 【硬 Gate 2.0 — 反转门控】三队 Stage 0 全部完成后 MUST 执行硬性阻断检查。Gate 状态机: IDLE→SCANNING→[有CRITICAL?→WAITING_ANSWER→BROADCAST]→PASSED。零问题零延迟自动通过。详见下方 Gate 规格。

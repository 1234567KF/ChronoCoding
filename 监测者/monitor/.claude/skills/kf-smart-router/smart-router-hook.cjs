#!/usr/bin/env node
/**
 * smart-router-hook.cjs — kf-smart-router PreToolUse Hook
 *
 * 在技能调用时注入智能路由决策。
 ...
 *   stderr: [kf-smart-router] 调试/状态信息
...
function main() {
  ...
  const hasSmartRouter = integrated.includes('kf-smart-router');
  ...
  if (hasSmartRouter || recommended || skillTypeMap[skillName]) {
    ...
    console.error(
      `[kf-smart-router] skill=${skillName} hasSmartRouter=${hasSmartRouter} rec="${recommended}" smart="${smartRecommendation}"`
    );

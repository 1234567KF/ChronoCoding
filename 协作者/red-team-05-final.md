# kf-smart-router — 全自动多模型智能调度系统 方案汇总
...
| 维度 | kf-model-router（现行） | kf-smart-router（增强） |
|------|------------------------|------------------------|
...
**关键设计决策**：kf-model-router 保留不动，kf-smart-router 作为增强层叠加。两者可同时注册 PreToolUse，互不冲突。

---

## 二、文件清单

### 核心模块（`D:\AICoding\.claude\skills\kf-smart-router\`）
...
增强: 协调者调用 kf-smart-router.dispatch({ description: "..." })
